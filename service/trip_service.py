...
def finish_running_trip(username: str, trip_id: int, image_path: Optional[str], lat, lng, plate_override=None, mileage_override=None, ocr_context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    回场登记：回场里程手动输入/确认值优先。
    ...
    final_plate = manual_plate or _normalize_plate(trip.get("plate")) or _normalize_plate(getattr(ocr, "plate", None))
    ...
    end_time = now_str()
    if image_path:
        image_path = rename_trip_photo(image_path, username, final_plate or "plate", end_time)
