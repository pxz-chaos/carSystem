...
def get_user_by_phone(phone: str) -> Optional[Dict[str, Any]]:
    ...
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT id, username, role, phone, email, email, gender, unit, department, team, unit_other, department_other, team_other, created_at FROM users WHERE phone=? LIMIT 1", (phone,))
    row = cur.fetchone()
    conn.close()
    return dict(row) if row else None
...
def get_user(username: str) -> Optional[Dict[str, Any]]:
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT id, username, role, phone, email, email, gender, unit, department, team, unit_other, department_other, team_other, created_at FROM users WHERE username=?", ((username or "").strip(),))
    row = cur.fetchone()
    conn.close()
    return dict(row) if row else None
...
def get_all_users() -> List[Dict[str, Any]]:
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT id, username, role, phone, email, email, gender, unit, department, team, unit_other, department_other, team_other, created_at FROM users ORDER BY id ASC")
    rows = cur.fetchall()
    conn.close()
...
def get_user_by_id(user_id: int) -> Optional[Dict[str, Any]]:
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT id, username, role, phone, email, email, gender, unit, department, team, unit_other, department_other, team_other, created_at FROM users WHERE id=?",
        (int(user_id),),
    )
    row = cur.fetchone()
    conn.close()
