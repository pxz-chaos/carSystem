...
from service.trip_service import finish_running_trip, preview_vehicle_ocr, start_trip
from service.maintenance_service import cleanup_export_files, cleanup_old_data, cleanup_orphan_upload_photos, get_storage_stats
from utils.export_utils import export_excel, export_images_zip
from utils.file_utils import save_upload_optional
from utils.sms_utils import SMS_PURPOSE_LOGIN, SMS_PURPOSE_RESET, generate_sms_code, send_sms_code
from config import EMAIL_CODE_TTL_SECONDS
...
def _send_and_store_email_code(email: str, purpose: str) -> None:
    ...
    # 复用 sms_codes 表存验证码，phone 字段中保存 email；purpose=reset_email，与短信验证码隔离。
    create_sms_code(phone=email, purpose=purpose, code=code, send_ip=_remote_ip())
        ...
        if request.method == "POST":
            ...
            try:
                ...
                if action == "send_code":
                    ...
                    flash(f"如果手机号已注册，验证码已发送，有效期 {SMS_CODE_TTL_SECONDS // 60} 分钟", "success")
 ...
 def forgot_password():
  if request.method == "POST":
   ...
   try:
    ...
    if not verify_sms_code(email, EMAIL_PURPOSE_RESET, code):
            ...
            try:
                if action == "confirm_ocr":
                    ...
                    try:
                        ...
                        return render_template("start_confirm.html", preview=pending.get("ocr") or {})
                ...
                if image_path:
                    ...
                    preview_for_session["raw_text"] = str(preview_for_session.get("raw_text") or "")[:1500]
            ...
            try:
                if action == "confirm_ocr":
                    ...
                    try:
                        ...
                        flash(str(confirm_exc), "danger")
                        return render_template("finish_confirm.html", trip=running_trip, preview=pending.get("ocr") or {})
                ...
                if image_path:
                    ...
                    return render_template("finish_confirm.html", trip=running_trip, preview=preview)
