...
def get_user_by_email(email: str) -> Optional[Dict[str, Any]]:
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT id, username, role, phone, email, gender, unit, department, team, unit_other, department_other, team_other, created_at "
...
def get_user_by_phone(phone: str) -> Optional[Dict[str, Any]]:
    ...
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT id, username, role, phone, email, gender, unit, department, team, unit_other, department_other, team_other, created_at FROM users WHERE phone=? LIMIT 1", (phone,))
    row = cur.fetchone()
    conn.close()
    return dict(row) if row else None
...
def get_user(username: str) -> Optional[Dict[str, Any]]:
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT id, username, role, phone, email, gender, unit, department, team, unit_other, department_other, team_other, created_at FROM users WHERE username=?", ((username or "").strip(),))
    row = cur.fetchone()
    conn.close()
    return dict(row) if row else None
...
def get_all_users() -> List[Dict[str, Any]]:
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT id, username, role, phone, email, gender, unit, department, team, unit_other, department_other, team_other, created_at FROM users ORDER BY id ASC")
    rows = cur.fetchall()
    conn.close()
...
def get_user_by_id(user_id: int) -> Optional[Dict[str, Any]]:
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT id, username, role, phone, email, gender, unit, department, team, unit_other, department_other, team_other, created_at FROM users WHERE id=?",
        (int(user_id),),
    )
    row = cur.fetchone()
    conn.close()
...
def delete_trip_record(record_id: int, username: Optional[str] = None) -> Optional[Dict[str, Any]]:
    """
    删除单条行程记录并返回被删除记录。

    - username=None：管理员删除任意记录；
    - username=当前司机：司机只能删除自己的记录。

    这里先查后删，调用方可根据返回记录删除对应照片。
    """
    try:
    ...
    cur = conn.cursor()
    if username:
        cur.execute("SELECT * FROM trip_records WHERE id=? AND username=?", (record_id, username))
    else:
        cur.execute("SELECT * FROM trip_records WHERE id=?", (record_id,))
    row = cur.fetchone()
    if row is None:
        conn.close()
        return None
    ...
    record = dict(row)
    if username:
        cur.execute("DELETE FROM trip_records WHERE id=? AND username=?", (record_id, username))
    else:
        cur.execute("DELETE FROM trip_records WHERE id=?", (record_id,))
    changed = cur.rowcount > 0
    conn.commit()
    conn.close()
