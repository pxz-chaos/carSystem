...
from utils.location_utils import format_location
...
def create_app():
    ...
    def forgot_password():
        if request.method == "POST":
            ...
            try:
                ...
                if not EMAIL_RE.match(email or ""):
                    raise ValueError("请输入注册时绑定的有效邮箱")
                ...
                if action == "send_code":
                    ...
                    flash(
                        f"邮箱验证码已发送，有效期 {EMAIL_CODE_TTL_SECONDS // 60} 分钟。若未收到，请检查垃圾箱或 debug_ocr/email_codes.txt",
                        "success",
                    )
                ...
                if not verify_email_code(email, EMAIL_PURPOSE_RESET, code):
                    raise ValueError("邮箱验证码错误、已使用或已过期")
        ...
        if request.method == "POST":
            ...
            try:
                if action == "set_email":
                    ...
                    if not EMAIL_RE.match(email or ""):
                        raise ValueError("请输入有效邮箱")
                    if not verify_user_password(session["username"], current_password):
                        raise ValueError("当前登录密码错误，邮箱未修改")
                    owner = get_user_by_email(email)
                    if owner and owner.get("username") != session["username"]:
                        raise ValueError("该邮箱已被其他账号绑定")
                    set_user_email(session["username"], email)
                    flash("邮箱已保存，可用于邮箱验证码找回密码", "success")
    ...
    def _delete_record_photos(record) -> int:
        """删除行程关联照片，只允许删除 UPLOAD_DIR 内的文件。"""
        deleted = 0
        try:
        ...
        for key in ("start_photo", "end_photo"):
            photo = (record or {}).get(key)
            if not photo:
                continue
            try:
                photo_path = os.path.abspath(str(photo))
                # 防止误删 uploads 之外的文件。
                if not (photo_path == base_dir or photo_path.startswith(base_dir + os.sep)):
                    continue
    ...
    def delete_record():
        ...
        delete_photos = request.form.get("delete_photos", "1") == "1"

        try:
            if not record_id.isdigit():
                raise ValueError("请选择要删除的行程记录")
            if not login_password:
                raise ValueError("请输入当前登录密码后再删除")
            if not verify_user_password(session.get("username", ""), login_password):
                raise ValueError("登录密码错误，删除操作已取消")

            # 管理员可以删除任意司机行程；司机只能删除自己的行程。
            owner = None if is_admin() else session.get("username", "")
            deleted_record = delete_trip_record(int(record_id), owner)
            if not deleted_record:
                raise ValueError("记录不存在，或你无权删除该行程")

            photo_count = _delete_record_photos(deleted_record) if delete_photos else 0
            if is_admin():
                flash(
                    f"已删除行程 #{deleted_record.get('id')}（司机：{deleted_record.get('username') or '-'}），同时删除照片 {photo_count} 张。",
                    "success",
                )
            else:
                flash(f"已删除我的行程 #{deleted_record.get('id')}，同时删除照片 {photo_count} 张。", "success")
        except Exception as exc:
            flash(str(exc), "danger")
